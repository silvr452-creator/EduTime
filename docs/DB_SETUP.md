...
## 5) Готовый backend в репозитории

- Папка: `backend/`
- Сервер: Node.js + Express + `pg`
- Эндпоинты реализованы: `GET /api/schedule/reference-data`, `GET/POST/PUT/DELETE /api/schedule/lessons`
- SQL-инициализация PostgreSQL: `backend/sql/init.sql`
- Переменные окружения backend: `backend/.env.example`
